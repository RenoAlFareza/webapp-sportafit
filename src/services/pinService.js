...
export const updatePin = async (oldPin, newPin) => {
  try {
    ...
    // karena backend mungkin belum siap
    const savedPin = localStorage.getItem('user_pin');
    console.log('Current saved PIN:', savedPin);

    if (savedPin === oldPin) {
      console.log('Old PIN matches, updating to new PIN');
      localStorage.setItem('user_pin', newPin);

      // Verifikasi bahwa PIN baru telah disimpan
      const newSavedPin = localStorage.getItem('user_pin');
      console.log('New saved PIN:', newSavedPin);

      return true;
    } else {
      console.log('Old PIN does not match saved PIN');
      return false;
    }
    ...
    */
    ...
    // Fallback ke localStorage untuk kompatibilitas
    const savedPin = localStorage.getItem('user_pin');
    console.log('In catch block - Current saved PIN:', savedPin);

    if (String(savedPin) === String(oldPin)) {
      console.log('In catch block - Old PIN matches, updating to new PIN');
      localStorage.setItem('user_pin', String(newPin));

      // Verifikasi bahwa PIN baru telah disimpan
      const newSavedPin = localStorage.getItem('user_pin');
      console.log('In catch block - New saved PIN:', newSavedPin);

      return true;
    }
    console.log('In catch block - Old PIN does not match saved PIN');
    return false;
  }
};
